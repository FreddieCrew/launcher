...
const StatItem = ({ label, value }: { label: string; value: React.ReactNode }) => (
  <div className="bg-bg-input p-2 rounded border border-border flex flex-col justify-center">
    <label className="text-[9px] text-gray-500 font-bold block uppercase tracking-wider mb-0.5">
      {label}
    </label>
  ...
  return (
    <div className="w-[320px] bg-bg-panel border-l border-border flex flex-col h-full">
      <div className="p-6 border-b border-border bg-[#18181b]">
        ...
        <div className="text-xs text-gray-500 font-mono mb-4 select-text">
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          <StatItem
            label="Players"
            value={`${selectedServer.players}/${selectedServer.maxPlayers}`}
          />
          <StatItem
            label="Ping"
            value={selectedServer.ping > -1 ? `${selectedServer.ping}ms` : '-'}
          />
          <StatItem label="Mode" value={selectedServer.mode} />
          <StatItem
            label="Version"
            value={selectedServerDetails ? selectedServerDetails.version || '-' : '...'}
          />
          <StatItem
            label="Map"
            value={selectedServerDetails ? selectedServerDetails.mapname || '-' : '...'}
          />
          <StatItem
            label="Lag Comp"
            value={
              selectedServerDetails
                ? selectedServerDetails.lagcomp === 'On'
                  ? 'Yes'
                  : 'No'
                : '...'
            }
        ...
        <div className="flex gap-2 py-2">
          ...
          <button
            ...
            className={clsx(
              ...
              selectedServer.isFavorite
                ? 'bg-accent hover:bg-accent-hover border-accent text-white'
                : 'border-border text-gray-500 hover:text-white hover:bg-accent-hover hover:bg-white/5'
            )}
          >
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {playerList.length === 0 ? (
            <div className="text-center py-10 text-xs text-gray-600 italic">
              No players online or loading...
            </div>
          ) : (
            playerList.map((p, i) => (
              <div
                key={i}
                className="flex justify-between text-xs text-gray-400 py-1 px-2 border-b border-white/5 last:border-0 hover:bg-white/5 rounded"
              >
                <span>{p.name}</span>
